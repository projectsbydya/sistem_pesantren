...
@section('title', 'Hafalan Kitab')

@section('content')
<div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Hafalan Kitab</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Rekaman hafalan kitab kuning santri</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 mb-6">
        ...
        <form method="POST" action="{{ tenant_route('dashboard.kepesantrenan.hafalan-kitab.store') }}" class="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
            @csrf
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Nama Kitab</label>
                <input type="text" name="nama_kitab" required class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm" placeholder="Fathul Qarib">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Bab / Halaman</label>
                <div class="flex gap-1">
                    <input type="text" name="bab" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm" placeholder="Bab">
                    <input type="text" name="halaman" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm" placeholder="Hal">
                </div>
            </div>
            <div>
            ...
            <div>
                ...
                <select name="status" required class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm">
                    @foreach(\App\Models\HafalanKitab::STATUS as $st)
                        <option value="{{ $st }}">{{ \App\Models\HafalanKitab::STATUS_LABELS[$st] }}</option>
                    @endforeach
                </select>
            </div>
            <div>
    ...
    @if($records->isEmpty())
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-1">Belum ada data hafalan kitab</h3>
        </div>
    @else
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                <thead class="bg-gray-50 dark:bg-gray-800/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Kitab</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Bab</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Halaman</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($records as $rec)
                        @php $color = \App\Models\HafalanKitab::STATUS_COLORS[$rec->status] ?? 'gray'; @endphp
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                            ...
                            <td class="px-4 py-2 text-sm text-gray-800 dark:text-gray-200">{{ $rec->nama_kitab }}</td>
                            <td class="px-4 py-2 text-sm text-gray-600 dark:text-gray-400">{{ $rec->bab ?? '-' }}</td>
                            <td class="px-4 py-2 text-sm text-gray-600 dark:text-gray-400">{{ $rec->halaman ?? '-' }}</td>
                            ...
                            <td class="px-4 py-2">
                                <span class="px-2 py-0.5 rounded-full text-xs font-bold bg-{{ $color }}-100 dark:bg-{{ $color }}-500/20 text-{{ $color }}-700 dark:text-{{ $color }}-400">
                                    {{ \App\Models\HafalanKitab::STATUS_LABELS[$rec->status] }}
                                </span>
                            </td>
                        </tr>
                    @endforeach
