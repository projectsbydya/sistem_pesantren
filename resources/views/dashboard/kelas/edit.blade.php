...
<div class="max-w-xl mx-auto">
    ...
    @php $selectedIds = old('subject_ids', $kelas->subjects->pluck('id')->toArray()); @endphp

    <form method="POST" action="{{ tenant_route('dashboard.akademik.kelas.update', ['programSlug' => $programSlug, 'id' => $kelas->id]) }}"
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $kelas->name) }}" required
        ...
        <div>
            ...
            <textarea name="description" rows="2"
                      class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 text-[13px] focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors resize-none">{{ old('description', $kelas->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            @if($subjects->isEmpty())
                <p class="text-[12px] text-gray-400 dark:text-gray-500 italic">Belum ada mata pelajaran.</p>
            @else
