...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-2">Edit Tagihan SPP</h1>
    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ $bill->santri?->name }}</p>
</div>
...
<x-card>
    ...
    @php
        $oldSantriId = old('santri_id', $bill->santri_id);
        $oldKelasId = $oldSantriId ? optional($santriList->firstWhere('id', (int) $oldSantriId))->kelas_id : null;
    @endphp

    <form method="POST" action="{{ tenant_route('dashboard.spp.update', ['bill' => $bill->id]) }}" class="space-y-5">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <select name="type" required
                ...
                @foreach(\App\Models\Bill::TYPE_LABELS as $val => $label)
                    <option value="{{ $val }}" {{ old('type', $bill->type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="amount" value="{{ old('amount', $bill->amount) }}" required min="1"
                       class="w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors">
