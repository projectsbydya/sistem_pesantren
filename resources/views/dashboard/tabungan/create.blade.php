...
@section('title', 'Catat Transaksi Tabungan')
...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-2">Catat Transaksi Tabungan</h1>
</div>
...
<x-card>
    ...
    <form method="POST" action="{{ tenant_route('dashboard.tabungan.store') }}" class="space-y-5">
        @csrf
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <label class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer transition-colors
                    {{ old('jenis') !== 'tarik' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'border-gray-200 dark:border-gray-700' }}">
                    <input type="radio" name="jenis" value="setor" class="accent-emerald-600"
                        {{ old('jenis', 'setor') === 'setor' ? 'checked' : '' }}>
                    <div>
                        <p class="text-sm font-semibold text-gray-900 dark:text-gray-100">Setoran</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Santri menabung</p>
                    </div>
                </label>
                <label class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer transition-colors
                    {{ old('jenis') === 'tarik' ? 'border-rose-500 bg-rose-50 dark:bg-rose-500/10' : 'border-gray-200 dark:border-gray-700' }}">
                    <input type="radio" name="jenis" value="tarik" class="accent-rose-600"
                        {{ old('jenis') === 'tarik' ? 'checked' : '' }}>
                    <div>
                        <p class="text-sm font-semibold text-gray-900 dark:text-gray-100">Penarikan</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Santri mengambil</p>
                    </div>
                </label>
            </div>
