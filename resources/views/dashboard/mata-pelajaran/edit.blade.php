...
@section('title', 'Edit Mata Pelajaran ' . strtoupper($programSlug))
@section('breadcrumb')
...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-2">Edit Mata Pelajaran {{ strtoupper($programSlug) }}</h1>
</div>
...
<x-card>
    ...
    <form method="POST" action="{{ tenant_route('dashboard.akademik.subjects.update', ['programSlug' => $programSlug, 'id' => $subject->id]) }}" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $subject->name) }}" required autofocus
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors resize-none">{{ old('description', $subject->description) }}</textarea>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <x-btn type="submit" variant="primary" icon="fa-save">Perbarui</x-btn>
        </div>
    </form>
</x-card>
