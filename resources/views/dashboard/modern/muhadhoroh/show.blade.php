...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
    <x-stat-card title="Total {{ $typeLabel }}" value="{{ $records->count() }}" icon="{{ $icon }}" color="{{ $isMuhadhoroh ? 'purple' : 'indigo' }}" />
    ...
    @if($records->isEmpty())
        <div class="text-center py-16 px-6">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-5 max-w-sm mx-auto">
                Belum ada sesi {{ $isMuhadhoroh ? 'muhadhoroh' : 'public speaking' }} yang direkam untuk {{ $santri->name }}.
            </p>
                    ...
                    <tr class="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50/60 dark:hover:bg-gray-800/30 transition-colors group">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if($rec->is_video_submission)
                                ...
                                @else
                                    <span class="text-[11px] text-purple-500 dark:text-purple-400"><i class="fa-solid fa-video"></i></span>
                                @endif
                            @else
                                <span class="text-gray-300 dark:text-gray-600 text-[12px]">—</span>
                            @endif
