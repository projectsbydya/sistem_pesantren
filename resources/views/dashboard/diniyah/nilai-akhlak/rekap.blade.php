...
<div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Rekap Nilai Akhlak</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $program->name }} — Periode: {{ $from }} s/d {{ $to }}</p>
        </div>
        <a href="{{ tenant_route('dashboard.diniyah.nilai-akhlak.index', ['programSlug' => $programSlug]) }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg text-sm">
