...
@section('title', 'Nilai Akhlak - ' . $program->name)

@section('content')
<div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Nilai Akhlak</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $program->name }} &mdash; Penilaian akhlak santri</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 mb-6">
        ...
        <form method="POST" action="{{ tenant_route('dashboard.diniyah.nilai-akhlak.store', ['programSlug' => $programSlug]) }}" class="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="text" name="aspect" required maxlength="100" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm" placeholder="Penilaian Karakter...">
            </div>
            <div>
    ...
    @if($records->isEmpty())
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-12 text-center">
            <div class="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-4">
                <i class="fa-solid fa-award text-gray-400 dark:text-gray-500 text-2xl"></i>
            </div>
            <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-1">Belum ada data nilai akhlak</h3>
        </div>
    @else
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($records as $rec)
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                            ...
                            <td class="px-4 py-2">
                                <form method="POST" action="{{ tenant_route('dashboard.diniyah.nilai-akhlak.destroy', ['programSlug' => $programSlug, 'id' => $rec->id]) }}" onsubmit="return confirm('Hapus record ini?')">
                                    @csrf @method('DELETE')
