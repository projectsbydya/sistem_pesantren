...
@section('title', 'Monitoring Akhlak - ' . $program->name)

@section('content')
<div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Monitoring Akhlak</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $program->name }} &mdash; Pemantauan akhlak santri</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 mb-6">
        ...
        <form method="POST" action="{{ tenant_route('dashboard.diniyah.monitoring-akhlak.store', ['programSlug' => $programSlug]) }}" class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            @csrf
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Aspek Akhlak</label>
                <input type="text" name="aspect" required maxlength="100" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm" placeholder="Jujur, Amanah...">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Kategori</label>
                <select name="category" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm">
                    <option value="">Pilih...</option>
                    @foreach(\App\Models\DiniyahMonitoring::AKHLAK_CATEGORIES as $key => $label)
            ...
            <div>
                ...
                <select name="status" required class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm">
                    @foreach(\App\Models\DiniyahMonitoring::AKHLAK_STATUSES as $st)
                        <option value="{{ $st }}">{{ \App\Models\DiniyahMonitoring::AKHLAK_STATUS_LABELS[$st] }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Skor (1-4)</label>
                <input type="number" name="score" min="1" max="4" value="3" required class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 text-sm">
            </div>
            <div class="md:col-span-3">
                <button type="submit" class="px-6 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700">
    ...
    <form method="GET" action="{{ tenant_route('dashboard.diniyah.monitoring-akhlak.index', ['programSlug' => $programSlug]) }}"
        ...
        @if($kelasId)
            <a href="{{ tenant_route('dashboard.diniyah.monitoring-akhlak.index', ['programSlug' => $programSlug]) }}"
    ...
    @if($records->isEmpty())
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-1">Belum ada data monitoring akhlak</h3>
        </div>
    @else
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($records as $rec)
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                            ...
                            <td class="px-4 py-2">
                                <form method="POST" action="{{ tenant_route('dashboard.diniyah.monitoring-akhlak.destroy', ['programSlug' => $programSlug, 'id' => $rec->id]) }}" onsubmit="return confirm('Hapus record ini?')">
                                    @csrf @method('DELETE')
