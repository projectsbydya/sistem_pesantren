...
<div class="max-w-xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900 dark:text-gray-100 mt-2">Edit Penugasan Mengajar — {{ $program->name }}</h1>
    </div>
    ...
    <form method="POST" action="{{ tenant_route('dashboard.akademik.penugasan.update', ['programSlug' => $programSlug, 'id' => $penugasan->id]) }}"
        ...
        <div>
            ...
            <select name="ustadz_id" required
                ...
                @foreach($ustadz as $u)
                    <option value="{{ $u->id }}" {{ old('ustadz_id', $penugasan->ustadz_id) == $u->id ? 'selected' : '' }}>
                        {{ $u->user?->name ?? '-' }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="kelas_id" id="kelas-select" required
                ...
                @foreach($kelasList as $k)
                    <option value="{{ $k->id }}" data-subjects='{{ json_encode($k->subjects->map(fn($s) => ["id" => $s->id, "name" => $s->name, "code" => $s->code]), JSON_HEX_APOS | JSON_UNESCAPED_UNICODE) }}' {{ old('kelas_id', $penugasan->kelas_id) == $k->id ? 'selected' : '' }}>
                        {{ $k->name }}
                    </option>
                @endforeach
            </select>
        ...
        <script>
            (function() {
                ...
                const selectedSubjectId = {{ old('subject_id', $penugasan->subject_id) }};
                
                console.log('[Penugasan Edit] Script executed');
                console.log('[Penugasan Edit] kelasSelect:', kelasSelect ? 'found' : 'NOT FOUND');
                console.log('[Penugasan Edit] subjectSelect:', subjectSelect ? 'found' : 'NOT FOUND');
                console.log('[Penugasan Edit] selectedSubjectId:', selectedSubjectId);
                
                if (!kelasSelect || !subjectSelect) {
                    console.error('[Penugasan Edit] Required elements not found!');
                    return;
                }
                
                function updateSubjects() {
                    console.log('[Penugasan Edit] updateSubjects() called');
                    console.log('[Penugasan Edit] selectedIndex:', kelasSelect.selectedIndex);
                    
                    const selectedOption = kelasSelect.options[kelasSelect.selectedIndex];
                    if (!selectedOption) {
                        console.log('[Penugasan Edit] No option at selectedIndex');
                        return;
                    }
                    
                    const subjectsData = selectedOption.getAttribute('data-subjects');
                    console.log('[Penugasan Edit] data-subjects raw:', subjectsData);
                    
                    let subjects = [];
                    if (subjectsData) {
                        try {
                            subjects = JSON.parse(subjectsData);
                            console.log('[Penugasan Edit] Parsed subjects:', subjects);
                        } catch (e) {
                            console.error('[Penugasan Edit] JSON parse error:', e.message);
                            console.error('[Penugasan Edit] Problematic data:', subjectsData);
                            subjects = [];
                        }
                    } else {
                        console.log('[Penugasan Edit] No data-subjects attribute');
                    }
                    ...
                    if (!Array.isArray(subjects) || subjects.length === 0) {
                        ...
                        console.log('[Penugasan Edit] No subjects - showing empty hint');
                    } else {
                        ...
                        console.log('[Penugasan Edit] Populated', subjects.length, 'subjects');
                    }
                }
                
                kelasSelect.addEventListener('change', function() {
                    console.log('[Penugasan Edit] Change event - value:', this.value);
                    updateSubjects();
                });
                console.log('[Penugasan Edit] Change listener attached');
                
                // Initial load - always run for edit page
                if (kelasSelect.selectedIndex > 0) {
                    console.log('[Penugasan Edit] Initial load for selected kelas');
                    updateSubjects();
                } else {
                    console.log('[Penugasan Edit] No kelas selected (index:', kelasSelect.selectedIndex, ')');
                }
            })();
        </script>
        @endpush
        ...
        <div class="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
            ...
            <x-btn type="submit" variant="primary" icon="fa-floppy-disk">Perbarui</x-btn>
        </div>
    </form>
</div>
